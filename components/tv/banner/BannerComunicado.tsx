...
export default function BannerComunicado({ comunicado }: Props) {
        ...
        <section className={`absolute inset-0 overflow-hidden text-white ${aparencia.fundo}`}>
            ...
            <div className="relative z-[2] flex h-full items-center px-[clamp(3rem,8vw,10rem)] pb-[clamp(3rem,8vh,7rem)] pt-[clamp(6rem,15vh,11rem)]">
                <div className="w-full max-w-[1450px]">
                    <div className={`inline-flex items-center gap-3 rounded-full border bg-white/10 px-5 py-2.5 backdrop-blur-md ${aparencia.borda}`}>
                    ...
                    <h2 className="mt-[clamp(1.5rem,4vh,3.5rem)] max-w-[1350px] text-[clamp(3rem,7.2vw,8.5rem)] font-black leading-[0.9] tracking-[-0.055em] text-white">
                        {comunicado.titulo}
                    </h2>

                    <div className={`mt-[clamp(1.4rem,3vh,2.5rem)] h-1.5 w-[clamp(5rem,9vw,9rem)] rounded-full ${aparencia.destaque}`} />

                    <p className="mt-[clamp(1.4rem,3.5vh,3rem)] max-w-[1280px] text-[clamp(1.3rem,2.45vw,2.8rem)] font-semibold leading-[1.2] text-white/88">
                        {comunicado.mensagem}
                    </p>
                </div>
            </div>

            <div className={`absolute inset-x-0 bottom-0 h-[clamp(0.45rem,1vh,0.8rem)] ${aparencia.destaque}`} />
        </section>
    )
}
